...
def get_edm(path, elem_prop='mat2vec', n_elements='infer', inference=False,
            verbose=True, target_col="target"):
    """
    ...
    if 'formula' not in df.columns.values.tolist():
        if 'composition' in df.columns.values.tolist():
    ...
    if not inference:
        df = df.groupby(by='formula',sort=False).mean().reset_index()  # mean of duplicates
...
class EDM_CsvLoader():
    ...
    def __init__(self, csv_data, batch_size=64,
        ...
        self.main_data = list(get_edm(self.csv_data, elem_prop='mat2vec',
                                      ...
                                      inference=inference,
                                      verbose=verbose, target_col=target_col))

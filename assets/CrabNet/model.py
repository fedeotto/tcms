...
from .utilities.utilities import (Lamb, Lookahead, RobustL1, BCEWithLogitsLoss,
                         EDM_CsvLoader, Scaler, DummyScaler, count_parameters)
from .utilities.optim import SWA
...
class Model():
    ...
    def load_data(self, file_name, batch_size=2**9, train=False):
        ...
        #if train ==True then inference == False
        
        inference = not train
        
        #EDM_CsvLoader is a data class containing crab vectors,
        ...
        # update n_elements after loading datasetm
        self.n_elements = data_loaders.n_elements
        
        #n_elements is the max number of elements found in compound over
        #passed dataset.

        data_loader = data_loaders.get_data_loaders(inference=inference)
        
        #this will extract target value from PyTorch DataLoader class.
    ...
    def train(self):
        ...
        #I think this method implements training routine over the batch.
        
        self.model.train()
        ti = time()
        minima = []
        
        #train_loader will be obtained by get_data_loaders(inference=False)
    ...
    def fit(self, 
        ...
        for epoch in range(epochs):
            ...
            if self.classification:
                ...
                # val_score = matthews_corrcoef(act_v, pred_v)
                # train_str = f'train MCC: {train_score:0.3f}'
                # val_str = f'val MCC: {val_score:0.3f}'
                train_score = f1_score(act_t.astype(int), pred_t)
                val_score = f1_score(act_v, pred_v)
                train_str = f'train F1: {train_score:0.3f}'
                val_str = f'val F1: {val_score:0.3f}'
                ...
                validation_scores = {"Loss": loss.cpu().detach().numpy(),
                                        "Acc": accuracy_score(act_v, pred_v),
                                        "F1": f1_score(act_v, pred_v),
                                        "MCC": matthews_corrcoef(act_v, pred_v)}
            ...
            training_type = "classification_target" if self.classification else "log_target"
    ...
    def save_network(self,
        ...
        #     os.makedirs('./CrabNet/models/trained_models', exist_ok=True)
        #     path = f'./CrabNet/models/trained_models/{model_name}'
        #     print(f'Saving network ({model_name}) to {path}')
        # else:
        #     path = f'trained_models/crabnet/{model_name}'
